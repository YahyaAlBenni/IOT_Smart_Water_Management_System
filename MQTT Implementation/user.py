...
def on_message(client, userdata, msg):
    if msg.topic == "user/notifications": #-----response from server when there is any notification -------
        print(f"Notification received: {msg.payload.decode()}")
    elif msg.topic == "user/response": #-----response when user press R and we dispaly all sensors data--------
